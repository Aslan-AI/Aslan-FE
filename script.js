...
// API Configuration
const API_BASE_URL = 'https://aslan-be.onrender.com';
const WS_BASE_URL = 'wss://aslan-be.onrender.com';
...
const endSession = async () => {
    ...
    try {
        await fetch(`${API_BASE_URL}/api/sessions/${currentSession.id}/end`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
...
const generateAIResponse = async (userMessage, sentimentData) => {
...
// Message Handling
const addMessage = async (content, isUser = false, emotion = null, colorFlag = null) => {
    try {
        // First, validate message on the backend
        const validationResponse = await fetch(`${API_BASE_URL}/api/messages/validate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        const messageDiv = createMessage(content, isUser, emotion, colorFlag);
    ...
    try {
        ...
        if (!currentSession) {
            const sessionResponse = await fetch(`${API_BASE_URL}/api/chat/session`, {
                ...
                body: JSON.stringify({ client_id: localStorage.getItem('userId') })
            });
        ...
        const analysis = messageData.analysis;
        ...
        const response = await generateAIResponse(message, analysis);
        chatMessages.removeChild(typingIndicator);
        
        // Add AI response with emotion from analysis
        const emotionMap = {
            'POSITIVE': 'happy',
            'NEGATIVE': 'sad',
            'NEUTRAL': 'calm'
        ...
        addMessage(response, false, emotionMap[analysis.sentiment] || 'neutral', 
                  analysis.confidence > 0.7 ? 'green' : 
                  analysis.confidence > 0.4 ? 'yellow' : 'red');
...
const loadChatHistory = async () => {
    try {
        ...
        data.chat_history.forEach(item => {
            ...
            if (item.analysis) {
                addMessage(item.analysis.coping_strategy, false, 
                          emotionMap[item.analysis.sentiment] || 'neutral',
                          item.analysis.confidence > 0.7 ? 'green' : 
                          item.analysis.confidence > 0.4 ? 'yellow' : 'red');
            }
        });
    } catch (error) {
...
const checkForAlerts = async (messageData) => {
    ...
    if (severity.needsAlert) {
        try {
            await fetch(`${API_BASE_URL}/api/alerts/create`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
...
loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Update UI
        document.querySelector('.user-name').textContent = `Welcome, ${data.user.username}`;
...
registerForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const formData = {
        employeeId: registerForm.querySelector('input[name="employeeId"]').value,
        ...
        birthday: registerForm.querySelector('input[name="birthday"]').value
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!token || !user) {
        ...
        // Add active classes to default tab (login)
...
window.addEventListener('click', (e) => {
    const authModal = document.getElementById('authModal');
    if (e.target === authModal && localStorage.getItem('token')) {
